...
/// Draws the scrollable sidebar: services list followed by the packages section.
...
pub fn draw_service_list(state: &AppState, ui: &mut egui::Ui) -> Option<SidebarAction> {
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            // ── "Services" sticky-ish section header ──────────────────────────
            ...
            // ── "Packages & Executables" section ──────────────────────────────
            if !state.packages.is_empty() {
                draw_section_header(ui, "Packages & Executables");

                for (i, pkg) in state.packages.iter().enumerate() {
                    let selected = matches!(state.right_pane, RightPane::PackageDetail(idx) if idx == i);
...
fn draw_service_row(state: &AppState, ui: &mut egui::Ui, i: usize) -> Option<SidebarAction> {
    ...
    // A service row is "selected" only when the right pane is NOT a PackageDetail.
